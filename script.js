...
//выбор цвета трасы
document.getElementById('selectColor').addEventListener('change', chngColor);
...
function addSelect(color) {
    ...
    select.forEach(select => {
        ...
        for (var i = 0; i < allGame.length; i++) {
            if (allGame[i].color.includes(color)) {
...
// создаем трасу
function addTrack() { //принимает цвет трасы 
